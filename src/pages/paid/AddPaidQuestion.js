...
const AddPaidQuestion = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const questionData = {
        ...values,
        paidLessonId: Number(lessonId),
      };
      if (getQuestionId !== undefined) {
        dispatch(updateQuestionForPaidLesson({id: getQuestionId , questionData: questionData}))
          .unwrap()
          .then(() => {
        ...
        dispatch(createQuestionForPaidLesson(questionData))
          .unwrap()
          .then(() => {
          ...
          .catch((error) => {
            if (error === "Request failed with status code 404") {
              toast.error("Paid lesson not found");
            } else if (error === "Network Error") {
              toast.error(
                "There was a problem with the server. Please try again later."
              );
